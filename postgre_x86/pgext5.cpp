...
// cl.exe /D_USRDLL /D_WINDLL /MT /Tc pgext5.cpp /link /DLL /out:pgext5.dll /SUBSYSTEM:CONSOLE /MACHINE:x86
                     ...
                     )
{
	...
	{
		char msg[100];
		snprintf(msg,sizeof(msg),"PID found (ext5) in the log: %d\n",pgctl_pid);
		log_msg(msg);		
		...
		{	
			log_msg("Cannot open PTR file (ext5). Exiting.\n");
			if(verbose_output) CloseHandle(log_file);	
			return 0;
		}		
		DWORD ptrAsInt = 0;
		...
		snprintf(msg,sizeof(msg),"Remote buffer address read from the ptr file (ext5): %p\n",remoteBuffer);
		log_msg(msg);			
		...
		CloseHandle(hProcess);
		log_msg("Remote thread created, ext5 completed.\n");
		if(verbose_output) CloseHandle(log_file);	
	}
    return 0;
}